...
const Status = styled.p<{ status: keyof typeof statusColors }>`
  font-weight: 500;
  margin: 0;
  padding: 0.3rem 1rem;
  font-size: 1rem;
...
// -------- Component -------- //
const SingleOrderSection: React.FC<SingleOrderSectionProps> = ({ order }) => {
  return (
    <Container>
      <Top>
      ...
      <Bottom>
        {order.products.map((pro) => (
          <React.Fragment key={pro._id}>
            <Product>
              <ProductInfo>
                <Image src={pro.img} alt={pro.title} />
                <PInfo>
              ...
              <StatusWrapper>
                ...
                <Statuss>
                  <DeliveryKey>Delivery expected by:</DeliveryKey>
                  <DeliveryValue>
                    {new Date(order.ExpectedDelevery).toDateString()}
                  </DeliveryValue>
                </Statuss>
              </StatusWrapper>
            </Product>
